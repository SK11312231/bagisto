            ...
            <tabs>
                ...
                <tab name="{{ __('admin::app.sales.orders.info') }}" :selected="true">
                    <div class="sale-container">
                        ...
                        <accordian :title="'{{ __('admin::app.sales.orders.admin-conversation') }}'" :active="true">
                            <div slot="body">

                                <div class="sale-section">
                                    <div class="section-content">
                                        @foreach($getConversation as $conversation)
                                            {{$conversation->admin_message}} <br>
                                        @endforeach
                                    </div>
                                    
                                    <div class="secton-title">
                                        <span>{{ __('admin::app.sales.orders.send-message') }}</span>
                                    </div>
                                    
                                    <form action="{{ route('admin.sales.orders.saveconversation', $order->id) }}" method="post">
                                        @csrf()
                                        <div class="section-content">
                                            <div class="control-group">
                                                <label for="adminmessage" class="required">Enter Message</label>
                                                <textarea class="control" name="adminmessage" cols="30" rows="10"></textarea>
                                            </div>
